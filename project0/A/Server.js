...
/*
	This exithandler will go through all the active sessions
	and send a GOODBYE to all of them. After all that's done,
...
/*
	sends a command to the given remote. Call a callback if it is provided
*/
function sendMessage(remote, command, message,callback){
	//takes the structure from the message and override with the command
...
function closeSession(id,callback){
	...
	delete sessions[id];
	//this message is printed if a connection timeout or session is closed.
...
function processMsg(message,remote){
	...
	var cmd = msg[3];
	var seqNum = parseInt('0x'+msg.slice(4,8).toString('hex',0,4));
	...
	if(!(sesID in sessions)){
		...
		//for every missing packet, print LOST Packet
	...
	if(cmd == DATA){
		...
		//console.log("GOODBYE");
		process.stdout.write("0x" + sesID + " [" + seqNum +"] GOODBYE from client.\n" );
		closeSession(sesID);
	}else{
