...
function process_stdin_data(chunk) {
...
process.stdin.on('data', function(chunk) {
  // if we are in the first state, wait for half a second to get the hello back from server
  // will ignore data if state is stil not 1 or 2 after half second
  if (state === 0) {
    active += 1;
    var data_timer = setTimeout(function() {
      process_stdin_data(chunk);
    active -= 1;
    }, 500);
  } else {
    process_stdin_data(chunk);
